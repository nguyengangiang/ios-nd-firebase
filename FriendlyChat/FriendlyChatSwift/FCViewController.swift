...
class FCViewController: UIViewController, UINavigationControllerDelegate {
    ...
    func fetchConfig() {
        ...
        remoteConfig.fetch(withExpirationDuration: expirationDuration) { (status, error) in
            if status == .success {
                ...
                let friendlyMsgLen = self.remoteConfig["friendly_msg_len"]
                if friendlyMsgLen.source != .static {
                    self.msglength = friendlyMsgLen.numberValue
                    print("message length: \(self.msglength)")
                }
            } else {
                print("config not fetch \(error!)")
            }
        }
    ...
    func sendPhotoMessage(photoData: Data) {
        // pushes message w/ photo to the firebase database
        let imagePath = "chat_photos/" + Auth.auth().currentUser!.uid + "/\(Double(Date.timeIntervalSinceReferenceDate * 1000)).jpeg"
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if let imageUrl = message[Constants.MessageFields.imageUrl] {
            ...
            Storage.storage().reference(forURL: imageUrl).getData(maxSize: INT64_MAX) { (data, error) in
                ...
                if cell == tableView.cellForRow(at: indexPath) { DispatchQueue.main.async {
